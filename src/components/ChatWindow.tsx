    ...
    <div className="flex flex-col h-full max-w-4xl mx-auto bg-zinc-950/30 backdrop-blur-sm rounded-2xl border border-zinc-800/50 overflow-hidden shadow-2xl">
      ...
      <div className="p-4 border-bottom border-zinc-800 bg-zinc-900/50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Avatar className="w-10 h-10 border-2 border-indigo-500/50">
        ...
        <div className="space-y-6">
          <AnimatePresence initial={false}>
            ...
            {messages.map((msg, idx) => (
              ...
              >
                <Avatar className={cn(
                  ...
                  msg.role === "user" ? "border-zinc-700" : "border-indigo-500/30"
                )}>
                  {msg.role === "user" ? (
                    <AvatarFallback className="bg-zinc-800 text-zinc-400"><User className="w-4 h-4" /></AvatarFallback>
                  ) : (
                    <AvatarFallback className="bg-indigo-600 text-white">{personality.name[0]}</AvatarFallback>
                  )}
                </Avatar>
            ...
            {isLoading && (
              ...
              >
                <Avatar className="w-8 h-8 border-indigo-500/30">
                  <AvatarFallback className="bg-indigo-600 text-white">{personality.name[0]}</AvatarFallback>
                </Avatar>
      ...
      <div className="p-4 bg-zinc-900/80 backdrop-blur-md border-t border-zinc-800">
        <div className="flex gap-2 max-w-3xl mx-auto">
          <Input
            ...
            className="bg-zinc-800 border-zinc-700 focus:ring-indigo-500 text-zinc-100 h-12"
            disabled={isLoading}
          />
          <Button 
            onClick={handleSend}
            disabled={!input.trim() || isLoading}
            className="bg-indigo-600 hover:bg-indigo-500 h-12 w-12 rounded-full shrink-0 p-0"
          >
